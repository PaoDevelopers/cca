...
func TestPostgresV1ToV2MigrationIntegration(t *testing.T) {
	...
	t.Run("compound aliases expand and data is preserved", func(t *testing.T) {
		...
		loadIntegrationSQLFile(t, ctx, pool, "../../database/migrations/testdata/schema_v1.sql")
		...
		loadIntegrationSQLFile(t, ctx, pool, "../../database/migrations/002_multi_periods.sql")
	...
	t.Run("unsupported occupied period rolls back atomically", func(t *testing.T) {
		...
		err := execIntegrationSQLFile(t, ctx, pool, "../../database/migrations/002_multi_periods.sql")
...
//nolint:revive // Test helpers keep testing.T first to match Go test conventions.
...
func newIsolatedIntegrationPool(t *testing.T, databaseURL string) *pgxpool.Pool {
	...
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	loadIntegrationSQLFile(t, ctx, pool, "../../database/schema.sql")
	return pool
}
...
func execIntegrationSQLFile(
	...
	t.Helper()
	// #nosec G304 -- path is a repository-local fixture supplied only by test code.
