    ...
    <div class="space-y-8">
        <div v-for="(ccas, category) in groupedCCAs" :key="category">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-7xl">
                <CCACard v-for="cca in ccas" :key="cca.id" :cca="cca" :disable-client-restriction="disableClientRestriction" :updating-cca-id="updatingCcaId" :show-period="showPeriod" @toggle="emit('toggle', $event)"/>
            </div>
        </div>
    </div>
</template>
